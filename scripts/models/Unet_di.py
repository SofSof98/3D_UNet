...
class Unet_3D(nn.Module):
    ...
    def __init__(self, input_shape, output_shape, opt, num_classes = 2):
        ...
        # downsampling
        self.conv_block_1 = Conv_3D_block_di(self.in_channels, filters[0], filters[0], self.is_batchnorm, batchNormObject)
        self.conv_block_2 = Conv_3D_block_di(filters[0], filters[1], filters[1], self.is_batchnorm, batchNormObject)
        self.conv_block_3 = Conv_3D_block_di(filters[1], filters[2], filters[2], self.is_batchnorm, batchNormObject)
        

        
        self.center_block = Conv_3D_block_di(filters[2], filters[3], filters[3], self.is_batchnorm, batchNormObject)

        # upsampling
        self.up_concat3 = Up_Conv_3D_block_di(filters[3], filters[2], filters[2], self.is_batchnorm, batchNormObject, opt.dr)
        self.up_concat2 = Up_Conv_3D_block_di(filters[2], filters[1], filters[1], self.is_batchnorm, batchNormObject, opt.dr)
        self.up_concat1 = Up_Conv_3D_block_di(filters[1], filters[0], filters[0], self.is_batchnorm, batchNormObject, opt.dr)
