...
class Unet_3D(nn.Module):
    ...
    def __init__(self, input_shape, output_shape, opt, num_classes = 2,is_conv=False):
        ...
        # downsampling
        self.conv_block_1 = RCNN_block(self.in_channels, filters[0] // 2, filters[0], batchNormObject ,time=3,dr =opt.dr) 
        self.conv_block_2 = RCNN_block(filters[0], filters[0], filters[1], batchNormObject ,time=3,dr =opt.dr)
        self.conv_block_3 = RCNN_block(filters[1], filters[1], filters[2],batchNormObject ,time=3,dr =opt.dr)
        

        
        self.center_block = RCNN_block(filters[2], filters[2], filters[3],batchNormObject ,time=3,dr =opt.dr)
        self.gating = UnetGridGatingSignal3(filters[3], filters[3], (1, 1, 1),self.is_batchnorm, batchNormObject)
        # upsampling
        self.up_concat3 = Up_Conv_3D(filters[3], filters[3], filters[2], block =RCNN_block, batchNormObject=batchNormObject ,time=3,dr =opt.dr)
        self.up_concat2 = Up_Conv_3D(filters[2], filters[2], filters[1], block =RCNN_block, batchNormObject=batchNormObject ,time=3,dr =opt.dr)
        self.up_concat1 = Up_Conv_3D(filters[1], filters[1], filters[0], block =RCNN_block, batchNormObject=batchNormObject ,time=3,dr =opt.dr)
