...
class Unet_3D(nn.Module):
    ...
    def __init__(self, input_shape, output_shape, opt, num_classes = 2):
        ...
        # downsampling
        self.conv_block_1 = First_Res_Block(self.in_channels, filters[0] // 2, filters[0], batchNormObject, opt.dr)
        self.conv_block_2 = Residual_Block(filters[0], filters[0], filters[1], batchNormObject, opt.dr)
        self.conv_block_3 = Residual_Block(filters[1], filters[1], filters[2], batchNormObject, opt.dr)
        

        
        self.center_block = Residual_Block(filters[2], filters[2], filters[3], batchNormObject, opt.dr)

        # upsampling
        self.up_concat3 = Up_Conv_3D_Res(filters[3], filters[3], filters[2], batchNormObject, opt.dr)
        self.up_concat2 = Up_Conv_3D_Res(filters[2], filters[2], filters[1], batchNormObject, opt.dr)
        self.up_concat1 = Up_Conv_3D_Res(filters[1], filters[1], filters[0], batchNormObject, opt.dr)
